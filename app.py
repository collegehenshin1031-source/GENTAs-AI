...
def draw_wall_chart(ticker_data: Dict[str, Any]):
    ...
    # --- 1. 価格帯別出来高の集計 ---
    bins = 60 # 精度向上のためビン数を少し増やす
    p_min = min(hist['Close'].min(), current_price * 0.85)
    p_max = max(hist['Close'].max(), current_price * 1.15)
    ...
    # --- 2. 複数の壁（ピーク）を検出 ---
    # 単純な最大値だけでなく、極大値（山）を探す
    peaks = []
    vol_values = vol_profile.values
    price_values = [i.mid for i in vol_profile.index]
    
    # 簡易ピーク検出（隣より大きい場所）
    for i in range(1, len(vol_values) - 1):
        if vol_values[i] > vol_values[i-1] and vol_values[i] > vol_values[i+1]:
            # ある程度大きな山だけ採用（平均の0.5倍以上など）
            if vol_values[i] > np.mean(vol_values) * 0.5:
                peaks.append({'price': price_values[i], 'vol': vol_values[i]})

    # ピークがない場合は最大値だけ入れる
    if not peaks:
        max_idx = np.argmax(vol_values)
        peaks.append({'price': price_values[max_idx], 'vol': vol_values[max_idx]})

    # 現在値より「上」と「下」に分ける
    upper_walls = [p for p in peaks if p['price'] > current_price * 1.005] # 0.5%以上離れている
    lower_walls = [p for p in peaks if p['price'] < current_price * 0.995]

    # それぞれ出来高順ではなく「価格順」にソート
    # 上値：近い順（昇順）
    upper_walls.sort(key=lambda x: x['price'])
    # 下値：近い順（降順）
    lower_walls.sort(key=lambda x: x['price'], reverse=True)
    ...
    if upper_walls:
        ...
        # 2. その上の抵抗線（点線：予備）
        if len(upper_walls) > 1:
            lines_to_draw.append({
                'y': upper_walls[1]['price'],
                'color': '#ef4444',
                'dash': 'dash', # 点線
                'text': '🧱 次の抵抗帯',
                'pos': 'top right'
    ...
    if lower_walls:
        lines_to_draw.append({
            'y': lower_walls[0]['price'],
            'color': '#3b82f6',
            'dash': 'dash', # 点線（突破済み感）
            'text': '👑 突破・支持線',
            'pos': 'bottom left'
        })
        # 4. その下の支持線（実線か点線：下値割り込みライン）
        if len(lower_walls) > 1:
            lines_to_draw.append({
                'y': lower_walls[1]['price'],
                'color': '#3b82f6',
                'dash': 'dot', # 細かい点線
                'text': '🛡️ 最終防衛線',
                'pos': 'bottom right'
...
def bundle_to_df(bundle: Any, codes: List[str]) -> pd.DataFrame:
    ...
    show_cols = [
        ...
        "大口介入", "浮動株・激動率", "異常・着火倍率", 
        "需給の壁", 
        "詳細", 
        "配当利回り", "年間配当", "事業の勢い", "業績", "時価総額", "根拠【グレアム数】"
    ]
...
- **👑 突破・支持線（旧・抵抗帯）**
    - **【基本】** かつて抵抗線だった壁が、今は**「点線の支持線（床）」**に変わった状態。ここが新たな防衛ラインになります。
    - **【割込】** ここを割り込むと、防衛ライン決壊（ダマシ）となり、一気に崩れる危険があります。
- **🔥 激戦中（分岐点）**
    - まさに今、その壁の中で戦っている。突破するか、跳ね返されるか、要注目！

### 4. 新機能：ハゲタカ・ハント指標（大口検知）
- **🌪️ 浮動株・激動率（旧：回転率）**
    - ただの出来高ではありません。**「市場で売買可能な株（浮動株）」**に対して、どれだけ注文が殺到したかを監視します。
