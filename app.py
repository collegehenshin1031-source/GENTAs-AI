...
def format_market_cap(oku_val):
    oku_val = safe_float(oku_val, default=0.0)
    if oku_val <= 0:
        return "算出対象外"
...
def evaluate_stock(ticker):
    ...
    try:
        ...
        try:
            current_price = safe_float(hist['Close'].iloc[-1], default=float('nan'))
        ...
        mc = safe_float(info.get('marketCap'), default=0.0)
        if mc <= 0 and shares > 0:
            mc = current_price * shares
        market_cap_oku = mc / 1e8 if mc > 0 else 0.0
        ...
        insufficient_materials = (len(hist) < 60) or (market_cap_oku <= 0 and shares <= 0)
        ...
        result = {
            ...
            "現在値": int(round(current_price)), "時価総額_表示": format_market_cap(market_cap_oku),
                ...
                if not codes:
                    ...
                    for c in codes:
                        ...
                        if res["status"] == "not_found":
                            st.info(f"【 {c} 】 この銘柄は、現時点では判断材料が少ないため、源太AIの監視対象外です。")
                        
                        elif res["status"] == "insufficient_data":
                            st.info(f"【 {c} 】 {res.get('message', 'この銘柄は、現時点では判断材料が少ないため、源太AIの監視対象外です。')}")
                        
                        elif res["status"] == "error":
                            st.info(f"【 {c} 】 この銘柄は、現時点では判断材料が十分にそろっていないため、診断対象外となっています。")
